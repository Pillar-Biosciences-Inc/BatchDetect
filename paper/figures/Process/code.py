...
axA.scatter(xj, yy1, s=10, alpha=0.8, label="Sample 1")
axA.scatter(xj, yy2, s=10, alpha=0.8, label="Sample 2")
axA.axhline(0.0, linewidth=1.0, linestyle="--")
axA.set_title("A  Processed lCNR profiles", fontsize=12)
axA.set_xlabel("Amplicon index", fontsize=12)
axA.set_ylabel("lCNR", fontsize=12)
axA.legend(frameon=False, fontsize=9, loc="upper right")
style_axis(axA)

# Panel B: 200 points, similar look (sorted evidence scores)

x1 = rng.normal(size=N)*.3 - .2
x2 = rng.normal(size=N)*.3 - .4
x1[100] -= 2
ell1 = np.cumsum(x1)
ell2 = np.cumsum(x2)
...
axB.set_title("B  Compute Evidence Score", fontsize=12)
axB.set_xlabel("Amplicon Index", fontsize=12)
axB.set_ylabel("Accumuated Evidence", fontsize=12)
style_axis(axB)

# Panel C: color-coded histogram (by cluster assignment)
# Note: matplotlib will auto-assign different colors for each call to hist.
bins = 28
axC.hist(ell[labels == 0], bins=bins, alpha=0.75, label="Cluster 1")
axC.hist(ell[labels == 1], bins=bins, alpha=0.75, label="Cluster 2")
axC.set_title("C  Evidence Distribution", fontsize=12)
axC.set_xlabel("Evidence Score ", fontsize=12)
axC.set_ylabel("Count", fontsize=12)
axC.legend(frameon=False, fontsize=9, loc="upper left")
style_axis(axC)

# Panel D: leave alone (bootstrap LR null + observed LR)
axD.hist(LR_b, bins=22, alpha=0.9)
axD.axvline(10, linewidth=2.0)
axD.set_title("D  Bootstrap null for LR statistic", fontsize=12)
axD.set_xlabel("Bootstrap LR statistic", fontsize=12)
axD.set_ylabel("Count", fontsize=12)
style_axis(axD)
axD.text(
    0.02,
    0.98,
...
# Save
fig.savefig("figure_method_overview_4panel_v2.pdf")
fig.savefig("figure_method_overview_4panel_v2.png", dpi=200)
plt.show()
