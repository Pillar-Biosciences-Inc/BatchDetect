...
axA.set_title("Processed lCNR profiles", fontsize=TITLE_FS)
...
x1 = rng.normal(size=N) * 0.3 + 0.2
x2 = rng.normal(size=N) * 0.3 - 0.4
x1[100] -= 2
ell1 = np.cumsum(x1)
ell2 = np.cumsum(x2)

axB.plot(np.arange(1, N + 1), ell1, linewidth=3.0, color=PASTEL["purple"], alpha=0.95)
axB.plot(np.arange(1, N + 1), ell2, linewidth=3.0, color=PASTEL["green"], alpha=0.95)
axB.set_title("Compute Evidence Score", fontsize=TITLE_FS)
axB.set_xlabel("Amplicon Index", fontsize=LABEL_FS)
axB.set_ylabel("Accumulated Evidence", fontsize=LABEL_FS)
style_axis(axB)
...
axD.set_title("Bootstrap null for LR statistic", fontsize=TITLE_FS)
axD.set_xlabel("Bootstrap LR statistic", fontsize=LABEL_FS)
