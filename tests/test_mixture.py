    ...
    **kwargs
    ...
    for k in range(n_components):
        ...
        if distribution == "laplace":
            ...
            X[mask, 0] = gennorm.rvs(beta, loc=loc, scale=scale, size=nk, random_state=rng)
        elif distribution == "hypsecant":
            from scipy.stats import hypsecant
            X[mask, 0] = hypsecant.rvs(loc=loc, scale=scale, size=nk, random_state=rng)
            
    return X, z, weights, means, scales

@pytest.mark.parametrize("distribution", ["laplace", "gaussian", "student_t", "gennorm", "hypsecant"])
...
def test_parametric_bootstrap_lrt():
    ...
    def null_factory():
        return GaussianMixture(n_components=1, random_state=42)
