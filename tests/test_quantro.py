...
import pytest
import numpy as np
from batchdetect.quantro import quantro, _compute_Fnik
...
def test_quantro_input_validation():
    ...
    group_mismatch = np.array([0, 1]) # length 2, but X has 2 columns? Wait.
    # X is (n_features, n_samples). Code says:
    # nT = X.shape[1]
    ...
    with pytest.raises(ValueError, match="Number of columns in object does not match"):
        quantro(x_2d, group_mismatch_len)

    # 3. B < 0 -> expect ValueError
    group_match = np.array([0, 1])
    with pytest.raises(ValueError, match="Must pick B greater than or equal to 0"):
...
def test_quantro_basic_no_perm():
    ...
    keys = ['anova', 'MSbetween', 'MSwithin', 'quantroStat', 'quantroStatPerm', 'quantroPvalPerm']
    for k in keys:
        assert k in res
...
def test_quantro_options():
    ...
    res_no_norm = quantro(X, group, B=0, useMedianNormalized=False, verbose=False)
